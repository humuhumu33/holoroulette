// broker-door.mjs — the SERVERLESS signal door: presence + matchmaking + SDP
// gossip over free public MQTT-over-WebSocket brokers, content-blind.
//
...
// it, MIT; carried here via televoid web/broker-door.mjs): the broker routes
// opaque sealed frames on an UNGUESSABLE topic derived from the door secret.
// It sees ciphertext on a random-looking topic — never a member id, an SDP, or
// a pair key. Any broker works; several are raced; none is ours.
//
...
export function makeBrokerDoor({ secret, brokers = DEFAULT_BROKERS } = {}) {
  return async function open(room, self, onMsg) {
    ...
    async function deliver(payload) {
      ...
      } catch { return; }             // not sealed by a door holder → ignore
